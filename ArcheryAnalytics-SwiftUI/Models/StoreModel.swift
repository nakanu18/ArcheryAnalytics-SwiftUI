...
class StoreModel: ObservableObject, Codable {
    ...
    func createNewTuningRound(roundType: RoundType) -> Round {
        var newRound = Round()
        newRound.name = roundType.name
        newRound.stages = roundType.stages
        
        print("- StoreModel: createNewTuningRound: \(newRound.name)")
        tuningRounds.insert(newRound, at: 0)
        return newRound
        
    }
    
    func updateTuningRound(round: Round) {
        guard let index = tuningRounds.firstIndex(where: { $0.id == round.id }) else {
            return
        }

        print("- StoreModel: updateTuningRound: [\(round.id)] - \(round.name)")
        tuningRounds[index] = round
    }
}
